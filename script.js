...
function initBunnyPlayerBackground() {
  document.querySelectorAll('[data-bunny-background-init]').forEach(function (player) {
    ...
    // play attempt not tied to click. used by IO auto-resume.
    ...
    // single observer. runs for every player, autoplay or not.
    if (player._io) {
      try {
    ...
    var io = new IntersectionObserver(
      function (entries) {
        entries.forEach(function (entry) {
          ...
          if (inView) {
            // lazyload src on first view, always (not just autoplay)
            if (isLazyTrue && !isAttached) attachMediaOnce();

            // autoplay players: start on first view too.
            // manual players: only resume if IO paused them before (not user pause).
            var shouldPlay = autoplay
              ? lastPauseBy === 'io' || (video.paused && lastPauseBy !== 'manual')
              : lastPauseBy === 'io';
            ...
            // pause on exit, only mark 'io' if it was actually playing
            if (!video.paused && !video.ended) {
              lastPauseBy = 'io';
              video.pause();
            }
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  components.forEach((component) => {
    ...
    // autoplay off at init. Manual control via observer.
    const mainSwiper = new Swiper(swiperEl, {
      slidesPerView: useFade ? 1 : 'auto',
      loop: false,
      observer: true,
      observeParents: true,
      autoplay: {
      ...
      on: {
        ...
        slideChange: function () {
          setActiveTab(this.activeIndex % totalSlides);
          resetAllProgress();
          if (isInView) startProgress(this.activeIndex % totalSlides);
        },
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            // reset to slide 1 first time it enters view
            if (!hasEnteredOnce) {
              hasEnteredOnce = true;
            ...
            resetAllProgress();
            startProgress(mainSwiper.activeIndex % totalSlides);
